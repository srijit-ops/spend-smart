...
function AllTransactionHolder() {
  ...
  return (
    <div className='w-full py-5 px-6'>
      ...
      <h3 className='text-xl tracking-wide text-gray-400 mb-7'>🗓️ {dayjs(currentMonthYear).format('MMM, YYYY')}</h3>
        ...
        <tbody className="border-t-[16px] border-transparent">
          {
            typeof localStorage !== 'undefined' &&
              JSON.parse(localStorage.getItem("transactionData"))?.[currentMonthYear]?.transactions?.map((item, index)=>{
                ...
                <td className={`py-4 ${item.expenseType==='debit'?'text-red-500':'text-green-500'}`}>
                  {item.expenseType==='debit'?'-':'+'}{item.amount} /-
                </td>
                <td className="py-4 text-white">
